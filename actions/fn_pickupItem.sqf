...
*/
diag_log "pickupItem || Script Start";
private["_obj","_itemInfo","_itemName","_illegal","_diff","_expGain"];
_diff = 0;
...
{
	_string = format[localize "STR_NOTF_PickedEvidence",_itemName,[(life_illegal_items select _illegal) select 1] call life_fnc_numberText];
...
if(_diff <= 0) exitWith {_string = localize "STR_NOTF_InvFull"; life_HUD_notifs pushBack[_string,time,2];};
_obj setVariable["PickedUp",TRUE,TRUE];
if(_diff != _itemInfo select 1) then
{
	if(([true,_itemInfo select 0,_diff] call life_fnc_handleInv)) then
	{
		...
		_string = format["You have picked %1 %2 up off the ground.",_diff,_itemName];
		life_HUD_notifs pushBack [_string,time,1];
	};
}
	else
